...
import { CONFIG } from './config.js?v=20260914-1';
import { PointCloudShader } from './shaders.js?v=20260914-1';
...
export class ArtEngine {
  ...
  async loadSceneData() {
    if (CONFIG.image.mediaType === 'video') {
      return new Promise((resolve) => {
        ...
        video.onloadeddata = () => {
          ...
          this.buildPointCloud(video);
          video.play().catch(() => {});
          resolve(true);
        };
        video.onerror = () => {
          console.warn('视频加载失败，使用占位图');
          this.buildPointCloud(this.createFallback());
          resolve(false);
        };
        video.src = CONFIG.image.src;
        video.load();
      });
    }
    return new Promise((resolve) => {
  ...
  updateVideoFrame(elapsed) {
    if (!this.videoSource || !this.colorAttribute || elapsed - this.lastVideoFrame < 1 / 20) return;
    if (this.videoSource.readyState < 2 || this.videoSource.paused) return;
    this.lastVideoFrame = elapsed;
    const w = CONFIG.image.sampleWidth;
    const h = CONFIG.image.sampleHeight;
    this.sampleContext.drawImage(this.videoSource, 0, 0, w, h);
    const pixels = this.sampleContext.getImageData(0, 0, w, h).data;
    const colors = this.colorAttribute.array;
    for (let source = 0, target = 0; source < pixels.length; source += 4, target += 3) {
      colors[target] = pixels[source] / 255;
      colors[target + 1] = pixels[source + 1] / 255;
      colors[target + 2] = pixels[source + 2] / 255;
    }
  ...
  initInteraction() {
    const handleInput = (clientX, clientY) => {
      if (this.videoSource?.paused) this.videoSource.play().catch(() => {});
